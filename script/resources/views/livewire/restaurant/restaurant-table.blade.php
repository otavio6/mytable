<div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        <thead class="bg-white/5 border-b border-white/5">
                            <tr>
                                ...
                                <th scope="col"
                                    class="py-2.5 px-4 text-xs font-medium text-center text-gray-500 uppercase dark:text-gray-400">
                                    @lang('app.createdAt')
                                </th>
                                <th scope="col"
                        ...
                        <tbody class="divide-y divide-white/5" wire:key='member-list-{{ microtime() }}'>
                            ...
                            @forelse ($restaurants as $item)
                                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700"
                                    ...
                                    <td
                                        class="py-2.5 px-4 text-sm text-gray-900 whitespace-nowrap dark:text-white inline-flex gap-2 items-center">
                                        <img src="{{  $item->logoUrl }}" class="h-8" alt="App Logo" />
                                        <div class="flex flex-col items-start gap-y-1">
                                            <a href="{{ route('superadmin.restaurants.show', $item->hash) }}" wire:navigate
                                                class="underline underline-offset-1 font-medium">{{ $item->name }}</a>

                                            @if(module_enabled('Subdomain'))
                                                <div class="flex flex-col items-center gap-y-1">
                                                    <a href="https://{{ $item->sub_domain }}" target="_blank"
                                                        class="underline  flex items-center gap-x-1 underline-offset-1 font-normal">{{ $item->sub_domain }}
                                            ...
                                            <div class="inline-flex items-center gap-x-1">
                                                <span
                                                    class="text-xs text-gray-600 border dark:border-gray-600 dark:bg-gray-700 dark:text-gray-400 bg-gray-200 px-2 py-1 rounded-md">{{ $item->branches_count }}
                                                    @lang('modules.settings.branches')</span>
                                                @if ($item->approval_status == 'Pending')
                                                    <svg class="h-5 w-5 text-yellow-400" width="24px" height="24px"
                                                ...
                                                @elseif ($item->approval_status == 'Rejected')
                                                    <svg class="h-5 w-5 text-red-500" width="24px" height="24px"
                                                        viewBox="0 0 24 24" fill="currentColor"
                                                        xmlns="http://www.w3.org/2000/svg">
                                                        <g stroke-width="0" />
                                                        <title>Rejection Reason: {{ $item->rejection_reason }}</title>
                                                        <g stroke-linecap="round" stroke-linejoin="round" />
                                                        <g fill="currentColor">
                                                            <path opacity=".5"
                                                                d="M3 10.417c0-3.198 0-4.797.378-5.335.377-.537 1.88-1.052 4.887-2.081l.573-.196C10.405 2.268 11.188 2 12 2s1.595.268 3.162.805l.573.196c3.007 1.029 4.51 1.544 4.887 2.081C21 5.62 21 7.22 21 10.417v1.574c0 5.638-4.239 8.375-6.899 9.536C13.38 21.842 13.02 22 12 22s-1.38-.158-2.101-.473C7.239 20.365 3 17.63 3 11.991z" />
                                    ...
                                    <td
                                        class="py-2.5 px-4 text-sm text-gray-900 whitespace-nowrap dark:text-white flex flex-col items-start gap-y-1.5">
                                        ...
                                        <span wire:click="showChangePackage({{ $item->id }})"
                                            wire:key='package-update-{{ $item->id . microtime() }}'
                                            class="bg-gray-100 hover:bg-gray-200 text-gray-700 select-none text-xs cursor-pointer font-medium inline-flex items-center px-1.5 py-0.5 rounded  dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-400 border border-gray-500 ">
                                            <svg class="w-4 h-4 text-current" aria-hidden="true"
                                    ...
                                    <td class="py-2.5 px-4 text-base text-gray-900 whitespace-nowrap dark:text-white">
                                        @if ($item->is_active == true)
                                            <span
                                                class="bg-green-100 uppercase text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300">@lang('app.active')</span>
                                        @else
                                            <span
                                                class="bg-red-100 uppercase text-red-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-red-900 dark:text-red-300">@lang('app.inactive')</span>
                                        @endif
                                    </td>

                                    <td
                                        class="py-2.5 px-4 text-sm text-gray-900 whitespace-nowrap dark:text-white text-center">
                                        ...
                                        <x-dropdown align="right">
                                            <x-slot name="trigger">
                                                <button type="button"
                                                    ...
                                                    <svg class="w-2.5 h-2.5 ms-1" height="24" width="24" aria-hidden="true"
                                            ...
                                            <x-slot name="content">
                                                ...
                                                @if ($item->approval_status == 'Pending')
                                                    <x-dropdown-link
                                                        wire:click="confirmApprovalStatus({{ $item->id }}, 'Approved')"
                                                        class="text-green-500 dark:text-green-400">
                                                        @lang('app.approve')
                                                    </x-dropdown-link>

                                                    <x-dropdown-link
                                                        wire:click="confirmApprovalStatus({{ $item->id }}, 'Rejected')"
                                                        class="text-yellow-500 dark:text-yellow-400">
                                                        @lang('app.reject')
                                                    </x-dropdown-link>
                                                @endif

                                                <x-dropdown-link wire:click='showEditCustomer({{ $item->id }})'
                                                    wire:key='member-edit-{{ $item->id . microtime() }}'>
                                                    @lang('app.update')
                                                </x-dropdown-link>
                                                <x-dropdown-link wire:click="showDeleteCustomer({{ $item->id }})"
                                                    wire:key='member-del-{{ $item->id . microtime() }}'
    ...
    <div wire:key='customer-table-paginate-{{ microtime() }}'
        class="sticky bottom-0 right-0 items-center w-full p-4 glass rounded-3xl mt-4 border-white/5 flex justify-between shadow-2xl">
    ...
    <x-confirmation-modal wire:model="confirmDeleteCustomerModal">
        ...
        <x-slot name="footer">
            ...
            @if ($restaurant)
                <x-danger-button class="ml-3" wire:click='deleteCustomer({{ $restaurant->id }})'
    ...
    <x-dialog-modal wire:model.live="showRejectionReasonModal">
        ...
        <x-slot name="content">
            <x-textarea id="rejectionReason" class="block mt-1 w-full" data-gramm="false" name="rejectionReason"
                placeholder="{{ __('modules.restaurant.rejectionReasonPlaceholder') }}" rows="3"
