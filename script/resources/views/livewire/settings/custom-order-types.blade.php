<div>
    <div class="p-4">
        ...
        <form wire:submit="saveOrderTypes" class="space-y-6">
            ...
            {{-- Disable Order Type Popup Toggle --}}
            <div class="flex gap-x-3 items-center p-4 bg-gray-100 rounded-lg shadow-sm dark:bg-gray-700">
                <x-checkbox name="disableOrderTypePopup" id="disableOrderTypePopup"
                    wire:model.live='disableOrderTypePopup' class="mr-4" />
                <div class="flex-1">
                    <x-label for="disableOrderTypePopup" :value="__('modules.settings.disableOrderTypePopup')" class="!mb-1" />
                    <p class="text-sm text-gray-500 dark:text-gray-400">@lang('modules.settings.disableOrderTypePopupDescription')</p>
                </div>
            </div>

            {{-- Default Order Type Selection --}}
            @if ($disableOrderTypePopup)
                <div class="p-4 bg-gray-50 rounded-lg border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                    <x-label for="defaultOrderTypeId" :value="__('modules.settings.defaultOrderType')" class="!mb-2" />
                    <x-select
                        id="defaultOrderTypeId"
                        class="w-full"
                        wire:model="defaultOrderTypeId"
                    >
                        @foreach($this->enabledOrderTypes as $id => $name)
                            <option value="{{ $id }}">{{ $name }}</option>
                        @endforeach
                    </x-select>
                    <x-input-error for="defaultOrderTypeId" class="mt-1" />
                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">@lang('modules.settings.defaultOrderTypeDescription')</p>
                </div>
            @endif
