<div>
    <aside id="sidebar"
        class="group/sidebar sticky top-0 ltr:left-0 rtl:right-0 z-20 flex flex-col flex-shrink-0 w-20 hover:w-64 h-full font-normal transition-all duration-500 ease-in-out glass rounded-[3rem] border-white/[0.05] shadow-2xl overflow-hidden"
        aria-label="Sidebar">
        <div class="relative flex flex-col flex-1 min-h-0 pt-0 bg-transparent">
            <div class="flex flex-col flex-1 pt-8 pb-4 overflow-y-auto overflow-x-hidden scrollbar-hide">
                <div class="flex-1 px-4 space-y-2 bg-transparent">

                    @if (user()->hasRole('Admin_' . user()->restaurant_id))
                        @livewire('forms.change-branch')
                    @else
                        <div
                            class="text-sm text-gray-500 dark:text-gray-400 py-2 px-3 font-semibold flex items-center justify-between">
                    ...
                    <ul class="py-2 space-y-2">
                        ...
                        @if ($this->hasModule('Menu') || $this->hasModule('Menu Item') || $this->hasModule('Item Category'))
                            @if (user_can('Show Menu') || user_can('Show Menu Item') || user_can('Show Item Category'))
                                <x-sidebar-dropdown-menu :name='__("menu.menu")' icon='menu'
                                    :active='request()->routeIs(["menus.*", "menu-items.*", "item-categories.*", "item-modifiers.*", "modifier-groups.*"])'>
                        ...
                        @if ($this->hasModule('Area') || $this->hasModule('Table'))
                            @if (user_can('Show Area') || user_can('Show Table'))
                                <x-sidebar-dropdown-menu :name='__("menu.tables")' icon='table'
                                    :active='request()->routeIs(["areas.*", "tables.*", "qrcodes.index"])'>
                        ...
                        @else
                            @if ($this->hasModule('Order') || $this->hasModule('KOT'))
                                @if (user_can('Show Order') || user_can('Manage KOT'))
                                    <x-sidebar-dropdown-menu :name='__("menu.orders")' icon='orders'
                                        :active='request()->routeIs(["orders.*", "kots.*"])'>
                        ...
                        @if ($this->hasModule('Expense') && user_can('Show Expense'))
                            <x-sidebar-dropdown-menu :name='__("menu.expenses")' icon='expenses'
                                :active='request()->routeIs(["payments.expenses", "payments.expenseCategory"])'>
                        ...
                        @if ($this->hasModule('Payment'))
                            @if (user_can('Show Payments'))
                                <x-sidebar-dropdown-menu :name='__("menu.payments")' icon='payments'
                                    :active='request()->routeIs(["payments.index", "payments.due"])'>
                        ...
                        @if ($this->hasModule('Report'))
                            @if (user_can('Show Reports'))
                                <x-sidebar-dropdown-menu :name='__("menu.reports")' icon='reports'
            ...
            <div class="absolute bottom-0 left-0 justify-center w-full p-2 space-x-4 bg-white md:flex dark:bg-gray-800 rtl:space-x-reverse"
                sidebar-bottom-menu>
                <a href="{{ module_enabled('Subdomain') ? 'https://' . restaurant()->sub_domain : route('shop_restaurant', [restaurant()->hash]) }}"
                    target="_blank"
